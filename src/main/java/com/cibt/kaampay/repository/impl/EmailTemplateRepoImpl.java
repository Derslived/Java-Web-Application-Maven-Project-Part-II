...
public class EmailTemplateRepoImpl implements EmailTemplateRepository {
    ...
    public JDBCTemplate<EmailTemplate> template = new JDBCTemplate();
    ...
    public void update(EmailTemplate model) throws Exception {
        ...
        String sql = "update tbl_email_templates set email_title=?, slug_name=?,subject=?,body=?,created_date=CURRENT_TIMESTAMP, modified_by=? where id=?";
        template.update(sql, new Object[]{model.getTitle(), model.getSlug(), model.getSubject(), model.getBody(), model.getModifiedBy(), model.getId()});
